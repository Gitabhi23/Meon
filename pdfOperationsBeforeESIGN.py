...
conn_Emkay_admin = sqlite3.connect('Emkay_admin_digilocker.db' , detect_types=sqlite3.PARSE_DECLTYPES,check_same_thread=False)
import base64
import uuid
import os
import shutil
...
pdfOperationsBeforeEsignAPI = Blueprint('pdfOperationsBeforeEsignAPI', __name__)
...
@pdfOperationsBeforeEsignAPI.route('/rekycPDFmail/<clientcode>', methods=['POST'])
def rekycPDFmail(clientcode):
    clientcode=clientcode.lower()
    data=request.get_json()
    user_email=data['user_email']
    name=user_email.split("@")[0]
    print('inside rekycPDFmail')
    equity_hash=str(uuid.uuid4()).replace('-','')
    
    emails=conn_Emkay_digilocker.execute('SELECT new_email,clientimage,email,name,pan, mobilechangeyesno, emailchangeyesno FROM userdetails WHERE clientcode = "'+clientcode+'"').fetchall()
    if emails[0][0]=="" or emails[0][0]==None:
        email=emails[0][2]
    else:
        email=emails[0][0]
    pan=emails[0][4]
    if (emails[0][1] != '' and emails[0][1] != None) or emails[0][5] == 'yes' or emails[0][6] == 'yes':
        kra_hash=str(uuid.uuid4()).replace('-','')
        linkkra = '  <a href="'+url+'requestesignemail/rekycPDF/rekykra/'+clientcode+'/'+kra_hash+'" style="background: #0067ab; outline: none; color: #fff; font-size: 13px; border: 1px solid #0067ab; padding: .50rem .75rem; letter-spacing: 1px; text-transform: uppercase;">click on kra link</a>'
    else:
        kra_hash = ''
        linkkra = ''

    text='Please <a href="'+url+'requestesignemail/rekycPDF/rekycam/'+clientcode+'/'+equity_hash+'" style="background: #0067ab; outline: none; color: #fff; font-size: 13px; border: 1px solid #0067ab; padding: .50rem .75rem; letter-spacing: 1px; text-transform: uppercase;">click on equity link</a>'+linkkra+' to eSIGN your ReKYC form and related documents to complete Account modification Process.'
    body='''<html>
            <head>
              <title></title>
            </head>
    ...
    print(emails)
    if (emails[0][1] != '' and emails[0][1] != None) or emails[0][5] == 'yes' or emails[0][6] == 'yes':
        mailsending("rekyc_support@emkayglobal.in",'',"Link to ReKYC E-sign",body,email,['',''],"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",["static/rekycPDF/"+clientcode+"/finalrekycpdf/""Re_"+str(pan)+".pdf","static/rekycPDF/"+clientcode+"/finalrekycpdf/""Rekra_"+str(pan)+".pdf"])
    else:
        mailsending("rekyc_support@emkayglobal.in",'',"Link to ReKYC E-sign",body,email,['',''],"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",["static/rekycPDF/"+clientcode+"/finalrekycpdf/""Re_"+str(pan)+".pdf"])


    conn_Emkay_digilocker.execute("UPDATE userdetails SET changes='esign_pending', email_time_before_esign='"+str(datetime.datetime.now())+"', equity_hash='"+equity_hash+"', kra_hash='"+kra_hash+"' where clientcode = '"+clientcode+"'")
    conn_Emkay_digilocker.commit()

    conn_Emkay_admin.execute("INSERT INTO masteradmin(client_pan,user_email,user_name,Action,datetime) VALUES(?,?,?,?,?)" ,(pan,user_email,name,"Esign_mailsent",datetime.datetime.now()))
    conn_Emkay_admin.commit()    
...
@pdfOperationsBeforeEsignAPI.route('/rekycpdf/<clientcode>', methods=['GET','POST'])
def accountmodification(clientcode):
    ...
    bse_cash, bse_fo, bse_curr, bse_comm, bse_slb, nse_cash, nse_fo, nse_curr, nse_comm, nse_slb, bse_mf = '','','','','','','','','','',''
    ...
    sign_of_holder =       ''#"E-signed by:"+data[0][2]+" <br>Date: "+data[0][32]+"<br> Reason: Re-KYC "
    sign =       ''#"E-signed by:"+data[0][2]+" <br>Date: "+data[0][32]+"<br> Reason: Re-KYC "
    ...
    old_date         =      str(data[0][32]).split(" ")[0]
        ...
        <div style="padding:15px">
            <table style="margin: 0;margin-left: 20px;width: 100%">
                <tr>
                    ...
                    <th style="text-align: center;">New Annual Income As on '''+str(new_date)+'''</th>
                </tr>
                <tr>
...
<br><br><br><br><br><br><br>
    ...
    pdfkit.from_string(html,"static/rekycPDF/"+str(original_clientcode)+"/rekycamtest.pdf",options=options )
    ...
    if data[0][28]!="":
        ...
        pdf.add_page()
        pdf.image(data[0][28], x=10, y=10, w=100, h=100)
    ...
    # if data[0][29]!="":
    #     pdf.add_page()
    #     pdf.image(data[0][29], x=10, y=10, w=100, h=100)
    ...
    if data[0][30]!="":
        ...
        pdf.add_page()
        pdf.image(data[0][30], x=10, y=10, w=100, h=100)
        # pdf.set_font('Arial', 'B', 8)
        # pdf.y = 200
        # pdf.x = 10
        # #pdf.cell(40, 20,"E-signed by:"+data[0][2])
    ...
    if data[0][31]!="":
        pdf.add_page()
        pdf.image(data[0][31], x=10, y=10, w=100, h=100)
        # pdf.set_font('Arial', 'B', 8)
        # pdf.y = 200
        # pdf.x = 10
        # #pdf.cell(40, 20,"E-signed by:"+data[0][2])
    ...
    if data[0][66]!="" :
        ...
        if data[0][66]!=None:
            pdf.add_page()
            pdf.image(data[0][66], x=10, y=10, w=100, h=100)
            # pdf.set_font('Arial', 'B', 8)
            # pdf.y = 200
            # pdf.x = 10
            # #pdf.cell(40, 20,"E-signed by:"+data[0][2])
    ...
    mergedObject = PdfFileMerger()
    mergedObject.append(PdfFileReader("static/rekycPDF/"+str(original_clientcode)+"/rekycamtest.pdf", "rb"), import_bookmarks=False)
    if data[0][168] == 'yes':
    ...
    mergedObject.write("static/rekycPDF/"+str(original_clientcode)+"/rekycam.pdf")
...
@pdfOperationsBeforeEsignAPI.route('/secondpdfAPI/<clientcode>', methods=['GET','POST'])
def kraAPI(clientcode):
    ...
    dob   = str(data[0][60]).split("-")[2]+'-'+str(data[0][60]).split("-")[1]+'-'+str(data[0][60]).split("-")[0]
    pan             = data[0][3]
    ...
    for i in range(2):
        ...
        if i==0:
            ...
            can.drawImage(data[0][72], 476,630,width=68,height=90)
        ...
        output.write(outputStream)
        outputStream.close()
        mergedObject.append(PdfFileReader("static/rekycPDF/"+str(clientcode)+"/destination"+str(i)+".pdf", "rb"), import_outline=False)
        # if i == 33:
        #   break
    ...
    mergedObject.write("static/rekycPDF/"+str(clientcode)+"/rekykra.pdf")
...
# @pdfOperationsBeforeEsignAPI.route('/secondpdfAPI/<clientcode>', methods=['POST'])
# def kraAPI(clientcode):
    # clientcode=clientcode.lower()
    # original_clientcode=clientcode
    # print('inside secondpdfAPI')
    ...
    #     marital_status   = str(data[0][63])
    #     citizenship      = 'INDIAN'
    #     signature        = ''#"E-signed by:"+data[0][2]+" <br>Date: "+data[0][32]+"<br> Reason: Re-KYC <br>Location: India"
    ...
    #     sign_d       = ''#"E-signed by:"+data[0][2]+" <br>Date: "+data[0][32]+"<br> Reason: Re-KYC <br>Location: India"
...
# </head>
# <body>
# <div><div class="center bold"><img src='''+LOGO+''' width="200px" alt="" /></div><div class="center bold">Emkay Global Financial Services Ltd.</div><div class="center">Central KYC Registry | Know Your Customer(KYC) Application | Individual</div><!--    Index   -->
...
#         options = {'enable-local-file-access': None,'--page-size' :'A4'}
#         pdfkit.from_string(html,"static/rekycPDF/"+str(original_clientcode)+"/rekykratest.pdf",options=options )
...
#                 pdf.add_page()
#                 pdf.image(i, x=10, y=10, w=150, h=150)
#                 # pdf.image(font('Arial', 'B', 8)
#                 # pdf.y = 200
#                 # pdf.x = 10
#                 # pdf.cell(40, 20,"E-signed by:"+data[0][302])
#                 # pdf.y = 205
#                 # pdf.x = 10
...
#                 # pdf.y = 215
#                 # pdf.x = 10
#                 # pdf.celli, x=10, y=10, w=150, h=150)
#                 # pdf.set_(70, 20,"Location: India" )
...
#         mergedObject = PdfFileMerger()
#         mergedObject.append(PdfFileReader("static/rekycPDF/"+str(original_clientcode)+"/rekykratest.pdf", "rb"), import_bookmarks=False)
#         mergedObject.append(PdfFileReader("static/rekycPDF/"+str(original_clientcode)+"/pdf2.pdf", "rb"), import_bookmarks=False)
#         mergedObject.write("static/rekycPDF/"+str(original_clientcode)+"/rekykra.pdf")
#         os.remove("static/rekycPDF/"+str(original_clientcode)+"/pdf2.pdf")
...
cors = CORS(pdfOperationsBeforeEsignAPI)
